...
Die Erzählfunktion lädt ausschließlich Kampagnenname und -idee, Genre, Stimmung, Spannung, aktuellen Szenenbeginn und -ziel, Namen der Beteiligten, Titel offener relevanter Handlungsstränge sowie einen begrenzten Ausschnitt des jüngsten Szenendialogs. Höchstens 16 Nachrichten und 24.000 Zeichen verhindern unkontrolliertes Kontextwachstum. Wissenseinträge, Geheimnisse, Charakterdetails und das übrige Protokoll sind nicht Teil dieser Abfrage. Der Adapter fordert keine serverseitige Speicherung der Antwort an. Fehlerprotokolle enthalten nur Fehlerklasse und unkritische Kategorie, nicht Schlüssel, Prompt, Kontext oder Antwort.

Anbieterkataloge liegen hinter dem internen Port `AiModelCatalog`. Der OpenRouter-Adapter validiert die externe Modellliste und gibt nur Modell-ID, Anzeigename und Kostenklasse weiter. Die Oberfläche kann dadurch kostenlose Modelle filtern, ohne Anbieterantworten oder Schlüssel in Domainlogik zu übernehmen.
