...
Start, Nachricht, Protokolleintrag, Probe, Orakelfrage, Inspiration und Abschluss erzeugen `SCENE_STARTED`, `SCENE_MESSAGE_ADDED`, `SCENE_NOTE_ADDED`, `DICE_ROLLED`, `ORACLE_ANSWERED`, `ORACLE_INSPIRATION_DRAWN` und `SCENE_COMPLETED`. Die Chronik zeigt sie in der Kategorie „Szenen“. Spielzeit und KI-erzeugte Erzählantworten folgen in späteren Arbeitspaketen.
